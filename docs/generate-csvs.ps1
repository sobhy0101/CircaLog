...
function Convert-XLDate([double]$s) {
    return ([DateTime]::new(1899,12,30)).AddDays([int]$s).ToString("yyyy-MM-dd")
}

# Convert Excel time fraction to HH:MM (pass-through for "Bedtime" / "As needed" / "")
function Convert-XLTime($f) {
    if ($f -eq "Bedtime" -or $f -eq "As needed" -or $f -eq "" -or $null -eq $f) { return $f }
    $m = [int][Math]::Round([double]$f * 1440)
    return "{0:D2}:{1:D2}" -f ($m / 60), ($m % 60)
}
...
$sleepLines = @("Date,Bed Time,Sleep Start,Wake Time,Sleep Onset Latency,Sleep Duration,Time in Bed,Session Type,Quality,Had Dreams,Dream Notes,Interruptions,Notes,Cycle Number")
foreach ($r in $sleepRows) {
    $cols = @(
        Convert-XLDate $r[0],
        Convert-XLTime $r[1],
        Convert-XLTime $r[2],
        ...
        (Q $r[10]), (Q $r[11]), (Q $r[12]),
        $r[13]
    )
    $sleepLines += ($cols -join ",")
}
$sleepLines | Set-Content -Path "c:\Projects\CircaLog\docs\Sleep_Log_2026-05-29_to_2026-06-15.csv" -Encoding UTF8
Write-Host "Sleep Log: $($sleepRows.Count) rows written."
...
$medRows = @(
    ...
    @(46176,"Melatonin Copad","Bedtime",0.011111,"Taken","Took two pills because I have to sleep and wake up by 9 AM to buy things for my wife return to Egypt"),
...
$medLines = @("Date,Medication Name,Scheduled Time,Actual Time Taken,Status,Notes")
foreach ($r in $medRows) {
    $cols = @(
        Convert-XLDate $r[0],
        (Q $r[1]),
        (Convert-XLTime $r[2]),
...
$medLines | Set-Content -Path "c:\Projects\CircaLog\docs\Medication_Log_2026-05-29_to_2026-06-15.csv" -Encoding UTF8
Write-Host "Medication Log: $($medRows.Count) rows written."
...
$foodRows = @(
    ...
    @(46173,0.909722,"Lunch","Large","Vegetable soup + 2 large 1 med baladi bread",1.076389,"360h 11m"),
    ...
    @(46187,0.833333,"Lunch","Large","Koshary + Beef Lazania",1.0,"26h 1m")
)

$foodLines = @("Date,Meal Time,Meal Name,Meal Size,Notes,Safe to Sleep After,Hours Since Meal")
foreach ($r in $foodRows) {
    # Safe to Sleep After fraction > 1 means next day — take mod 1
    $safeAfterFrac = [double]$r[5] % 1.0
    $cols = @(
        Convert-XLDate $r[0],
        (Convert-XLTime $r[1]),
        $r[2],
        $r[3],
        (Q $r[4]),
        (Convert-XLTime $safeAfterFrac),
        $r[6]
    )
    $foodLines += ($cols -join ",")
}
$foodLines | Set-Content -Path "c:\Projects\CircaLog\docs\Food_Log_2026-05-29_to_2026-06-15.csv" -Encoding UTF8
Write-Host "Food Log: $($foodRows.Count) rows written."

# ── DASHBOARD ──────────────────────────────────────────────────────────────────
$dashboard = @"
# Dashboard Snapshot — 15 June 2026
...
- Sugarlo Plus (Metformin) | Scheduled: 10:00 | Status: Taken at 12:19
- Thiotacid 300 (Alpha-lipoic Acid) — 1st Dose | Scheduled: 10:00 | Status: Taken at 12:19
- Milga Advance | Scheduled: 16:00 | Status: Due — take now
- Davalindi (Vitamin D3) | Scheduled: 16:00 | Status: Due — take now
- Thiotacid 300 (Alpha-lipoic Acid) — 2nd Dose | Scheduled: 16:00 | Status: Taken at 16:00
- Thiotacid 300 (Alpha-lipoic Acid) — 3rd Dose | Scheduled: 22:00 | Status: Taken at 22:00
- Melatonin Copad | Scheduled: Bedtime | Status: Taken at 06:45
...
"@
$dashboard | Set-Content -Path "c:\Projects\CircaLog\docs\Dashboard_2026-06-15.md" -Encoding UTF8
Write-Host "Dashboard written."
